...
public class BattleSystem {
    ...
    public BattleSystem(ArrayList<Army> player1Team, List<Army> player2Team, String homeground) {
        ...
        for (int turn = 1; turn <= 11; turn++) {
            ...
            for (Army attacker : sortedArmies) {
                if (ground.highLanders.contains(attacker)){
                    String warplace = this.homeground;
    ...
    private void performAttack(Army attacker) {
        Army target = getTarget();
        if (attacker instanceof Healers) {
            healLowestHealth(attacker, getOwnTeam());  ////////////////////////
        } 
        else {
            reduceHealth(attacker, target);
        }
    }
    private Army getTarget() {
        List<Army> opponentTeam = this.player2Team;

        if (!opponentTeam.isEmpty()) {
            for(Army target : opponentTeam){
                if (target.getHealth()<=0){
                    continue;
                }
                for (Army potentialTarget : opponentTeam) {
                    if (potentialTarget.getDefence() < target.getDefence() && target.getHealth()>0 && potentialTarget.getDefence()!=0) {
    ...
    private void reduceHealth(Army attacker, Army target) {
        if (target != null) {
            ...
            System.out.println(attacker.getName() +" character's health: "+ attacker.getHealth());
            System.out.println(target.getName()+" character's health: " + target.getHealth());
            if (target.getHealth() <= 0) {
                System.out.println(target.getName() + " has died!");
                target.setDefence(-target.getDefence());
            }
        }
    }
    ...
    private void healLowestHealth(Army healer, List<Army> team) {
        ...
        //if (target != null) {
            ...
            target.setHealth(healingAmount);
        //}
    }

    private Army findLowestHealth(List<Army> team) {
        Army lowestHealth = team[0].getHealth;

        for (Army member : team) {
            if (member.getHealth() < lowestHealth.getHealth()) {
                lowestHealth = member;
            }
        }
